...
class ZhugePassPortHandler extends HandlerBase
{
    static $baseUrl = 'http://id.zhuge.com';
    static $newUserUri = 'Passport/Register/register';
    static $updateUserUri = 'Passport/User/userinfoModify';
    static $deleteUserUri = 'api/v1/auth/user/delete';

    static $logoutUrl = 'Passport/Login/logout';
    ...
    {
        $passCookie = $request->cookies->get('passport');
        $token      = json_decode(base64_decode($passCookie));//解析手机号

        $sso       = $this->sso;
        $parameter = ['phone' => $token->username];
        $url       = sprintf('%s/%s', self::$baseUrl, self::$logoutUrl);
        // 记录日志
        $this->logger->info(sprintf('Request zhugePassPort Logout User ; Request url %s', $url), [$parameter]);
        //请求退出接口
        $res = $this->httpUtils->post($url, $parameter);

        // 记录返回日志
        $this->logger->info('Response zhugePassPort Create User Result', [$res]);
        $res = json_decode($res, true);

        if (!strtolower($res['code']) == 'success') {
            $this->logger->error('Response zhugePassPort Create User Failure', [$res]);
            return false;
        }

        // sessionId
        if ($sessionId = $request->cookies->get($sso['cookieName']) && $passCookie) {
            // 清除缓存
            $key = sprintf('%s_%s', $sso['keyIndex'], $sessionId);
            $this->common->S($key);
            $request->cookies->remove($sessionId);
            $request->cookies->remove($passCookie);
        }
