...
* 
*/
class Rent extends AbstractServiceManager
{
        ...
        if (isset($groupId))
        {
            ...
            if ($config && $user->getMid() != 1)
            {
                ...
                if ($count < $rentdrenum)
                    ...
                    return self::get('core.common')->showMessage('操作失败，您已经超过最大日发布数量');
            } else
...
//         dump($result,$oldValid,time());die();
        //经纪人或经纪公司数量统计
        self::rentUpd($result, $oldValid);
    ...
    public function rentAdd($obj)
    {
        if (($obj->getValid())>time())
            self::_shuliang($obj, 1);  // 数量+1
    }

    public function rentUpd($obj, $oldValid)
    {
    ...
    public function refresh(array $data, $isPay = false)
    {
        ...
        if(!is_object($sale))
            throw new \LogicException("出租屋信息不存在或已被删除!");
...
//         $this->get('db.userinfo')->update($userinfo->getId(), array(), $userinfo, false);
    ...
    protected function _shuliang($obj, $operate)
    {
        if (method_exists($obj, 'getValid'))
        {
            $uid = array();
            $cid = array();
            $infos = $this->get('house.rent')->findOneBy(array('id'=>$obj->getId()), array(), false);
    ...
    protected function _gx($map, $operate, $isjjr = true)
    {
        ...
        if ($operate)
            if ($isjjr)
                $info->setRents($info->getRents()+1);
            else
                $info->setCrents($info->getCrents()+1);
        else
            if ($isjjr)
                $info->setRents($info->getRents()>0?$info->getRents()-1:0);
            else
                $info->setCrents($info->getCrents()>0?$info->getCrents()-1:0);
