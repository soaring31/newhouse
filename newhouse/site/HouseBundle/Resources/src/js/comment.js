/**
 * author : 
 * date   : 
 * name   : comment
 * modify : 2017-02-10
 */

/**
 * @module comment
 * @param {String} list jq选择器 点评的列表
 * @param {String} replyBtn jq选择器 回复按钮
 * @param {String} form jq选择器 点评表单
 * @param {String} url 请求的列表地址
 * @param {String} removeItem jq选择器 要移除的结构（回复的时候用到）
 * @param {String} loadMore jq选择器 加载更多
 * @param {object} load 加载时候的配置
 * @param {String} saveUrl 点评提交的地址
 * @param {object} callfn 表单提交后的回调函数 当callfn == 1 时会执行楼盘的评分回调
 *
 * @module comment.zhibo
 * @param {String} list jq选择器 直播的列表
 * @param {String} url 请求的列表地址
 * @param {String} loadMore jq选择器 加载更多
 * @param {String} orderby 排序字段
 * @param {String} ordertype 排序方法
 * @param {String} time 刷新频率
 * 
 * 
 * 
 * @example 调用
 * ```js
 *  comment.init({
 *       list: '#pl-list',
 *       url: '{{ ("intercomment/lplist")|U({checked: 1, id: id|default('') }) }}',
 *       loadMore: '#load-more',
 *       saveUrl: '{{save_url}}',
 *       removeItem: '.up',
 *       callfn : 1,
 *       load : {pageSize: 12}
 *   });                   
 * ```
 * @预览 地址?id=19
 * @example
 * ```html
 * http://192.168.1.7/newcore/web/app_dev.php/house/houses/dcomment?id=19
 * ```
 * @说明信息
 * 调用的时候，一般用comment.init({});
 * 如果要用评分的效果， 再执行这个 : comment.raty();
 * 如果要调用直播刷新效果，请单独执行以下代码：
 * comment.zhibo({
 *        url: '{{ ("clist/zblist")|U({checked:1, topic:urlId}) }}',
 *       list: '#zblist'
 *
 *    });
 * 
 * 
 */

define(['qqFace', 'common', 'jqmodal', 'cookie', 'jqraty'], function(require, exports, module) {
    (function(out) {
        module.exports = out;
    })({
        // 初始化函数
        init: function(opt) {
            var _self = this,
                _opt = _self.opt = $.extend(true, {}, {
                    list: null,
                    reply: '.replys',
                    replyBtn: '.wyhf',
                    form: '.comment-form',
                    url: null,
                    removeItem: null,
                    loadMore: '.load-more',
                    load: {},
                    saveUrl: null,
                    callfn: null
                }, opt),
                $list = $(_opt.list),
                $form = $(_opt.form);

            // 防止重复初始化
            if ($list.hasClass('inited')) {
                return;
            }
            $list.addClass('inited');
            // 对url进行转码
            _self.opt.url = $(document.createElement('div')).html(_self.opt.url).text();


            _self.loadMore(_opt.load);
            // 点击回复
            $list.off('.cmt').on('click.cmt', _opt.replyBtn, function() {
                var $replyBtn = $(this);
                var $formClone = $list.find('.form-clone');
                var $item = $replyBtn.closest('[data-cid]');
                // 查看回复的框
                var $replysWrap = $item.find('.replys-wrap');

                if (!$formClone.length) {
                    $formClone = $form.clone().addClass('form-clone');
                    if (_opt.removeItem) {
                        $formClone.find(_opt.removeItem).remove();
                    }
                }
                // 设置回复ID                                    
                $formClone.find('input[name="toid"]').val($item.data('cid'));
                // 先隐藏查看回复的框
                if($replysWrap.length > 0){
                    $replysWrap.slideUp();
                }
                if ($replyBtn.hasClass('opened')) {
                    $replyBtn.removeClass('opened');
                    $item.append($formClone.slideUp());
                } else {
                    $formClone.prev().find(_opt.replyBtn).removeClass('opened');
                    $replyBtn.addClass('opened');
                    $item.append($formClone.slideDown());
                }

                // 
                if (!$formClone.hasClass('inited')) {

                    // 为form绑定完成事件
                    $formClone.find('form').on('done', function() {
                            _self.loadMore();
                        })
                    // 重置tabindex
                    $formClone.find('[tabindex]').attr('tabindex', function() {
                        return $(this).attr('tabindex') * 1 + 100;
                    })

                    // 复制后要刷新下验证码
                    $formClone.find('img').click();

                    // 处理表情
                    var $btnC1 = $formClone.find('.btn-c');
                    var $btnC1Data = $btnC1.data();
                    $btnC1Data.assign = '.form-clone #content';
                    if ($btnC1.length) {
                        $btnC1.qqFace($btnC1Data);
                    }

                    $formClone.addClass('inited');
                };
                return false;
            })
            // 查看回复
            .on('click', _opt.reply, function() {
                var $replys = $(this);
                var $item = $replys.closest('[data-cid]');
                var $replysWrap = $item.find('.replys-wrap');
                var $formClone = $list.find('.form-clone');
                

                if (!$replysWrap.length) {
                    $.ajax({
                        url: _opt.url,
                        dataType: 'html',
                        data: {
                            toid: $item.data('cid') || ''
                        },
                    })
                    .done(function(result) {
                        $item.append('<div class="replys-wrap">' + _self.replace_em(result) + '</div>');
                    })
                    .fail(function() {
                    })
                    .always(function() {
                    });
                } else {
                    $replysWrap.slideToggle();
                };
                if($formClone.length > 0){
                    $formClone.slideUp();
                    $(_opt.replyBtn).removeClass('opened');
                }
                return false;
            })
                // 点击加载更多
            $(document).off('.loadMore').on('click.loadMore', _opt.loadMore, function() {
                var page = $list.find('[data-cid]').eq(-1).data();

                _self.loadMore({
                    pageIndex: page.pageIndex + 1
                })
            });


            // 提示完成后
            $form.find('form').on('done', function() {
                _self.loadMore();
                // on
                if (_opt.callfn) {
                    if (_opt.callfn == 1) {
                        // 如果是1，就用默认的回调函数
                        _self.raty(_self.ratyOpt);
                        _self.callfn();
                    } else {
                        _opt.callfn.call();
                    }

                };
            })
            // 处理表情
            var $btnC = $form.find('.btn-c');
            if ($btnC.length) {
                $btnC.qqFace($btnC.data());
            }
        },
        // params=> {pageSize: }
        loadMore: function(params) {
            var _self = this;
            var _opt = _self.opt;
            var _params = $.extend(true, {}, {
                pageIndex: 1,
                toid: 0,
                order: 'id|desc',
                is_append: true
            }, params);
            var $list = $(_opt.list);
            var $loadMore = $(_opt.loadMore);

            if (_params['is_append']) {
                $loadMore.data('title', $loadMore.html()).html('加载中。。。');
            }


            $.ajax({
                url: _opt.url,
                type: 'GET',
                dataType: 'html',
                data: _params
            })
            .done(function(result) {

                // 有参数时追加

                $list[_params['is_append'] && params ? 'append' : 'html'](_self.replace_em(result));


                // 显示加载更多
                var page = $list.find('[data-cid]').eq(-1).data();
                var isShow = page ? page.pageIndex < Math.ceil(page.pageCount / page.pageSize) : 0;
                $loadMore[isShow ? 'show' : 'hide']();
            })
            .fail(function() {
                // console.log("error");
            })
            .always(function() {
                $loadMore.html($loadMore.data('title'));
            });
        },
        replace_em: function(str) {
            str = str.replace(/\[em_([0-9]*)\]/g, '<img src="' + jsbase + '/house/images/face/$1.gif" border="0" />');
            return str;
        },
        raty: function(opt) {
            var _self = this;
            var opt = _self.ratyOpt = $.extend(true, {}, {
                path: ''
            }, opt);

            console.log(opt.path);
            $('.pf-left .item .item-start').raty({
                number: 5,
                path: opt.path,
                width: 150,
                size: 18,
                starHalf: 'star-half.png',
                starOff: 'star-off-big.png',
                starOn: 'star-on-big.png',
                targetType: 'number',
                score: 3,
                mouseover: function(score, evt) {
                    $(this).nextAll('.item-fen').html(score + '分');
                },
                mouseout: function(score, evt) {
                    if (score) {
                        $(this).nextAll('.item-fen').html(score + '分');
                    } else {
                        $(this).nextAll('.item-fen').html('暂无评分');
                    }
                },
                click: function(score, evt) {
                    var num = 0;
                    $(this).nextAll('.val').val(score);
                    $(this).nextAll('.item-fen').html(score + '分');
                    $('.pf-left .item .val').each(function(index, el) {

                        num += parseInt($(this).val());
                    });
                    $('.pf-right .overall span').html(num / 5);
                    $('.pf-right').children('.val').val(num / 5);
                }
            });
        },
        callfn: function() {
            var $item = $('.pf-left .item');
            $item.find('.item-fen').html('3分');
            $item.find('.val').val(3);
            var $overall = $('.pf-right');
            $overall.find('.overall span').html(3);
            $overall.find('.val').val(3);
        },


        // 直播刷新
        zhibo: function(opt) {
            var _self = this;
            var _opt = _self.opt = $.extend(true, {}, {
                loadMore: '.load-more',
                url: null,
                orderby: 'id',
                ordertype: 'desc',
                time: 60 * 1000
            }, opt);

            var $list = $(_opt.list);

            if ($list.hasClass('inited')) {
                return;
            }
            $list.addClass('inited');

            _self.opt.url = $(document.createElement('div')).html(_self.opt.url).text();

            _self.loadMore({
                order: _opt.orderby + '|' + _opt.ordertype,
                is_append: false

            });

            var timer = setInterval(function() {
                _self.loadMore({
                    order: _opt.orderby + '|' + _opt.ordertype,
                    is_append: false

                });
            }, _opt.time);

            $(document).on('change', '#refresh', function() {
                _opt.time = $(this).find('option:selected').val() * 60 * 1000;
                clearInterval(timer);
                if (_opt.time) {
                    _opt.time = setInterval(function() {

                        _self.loadMore({
                            order: _opt.orderby + '|' + _opt.ordertype,
                            is_append: false

                        });
                    }, _opt.time);
                }
            })
            .on('change', '#orderby', function() {
                _opt.ordertype = $(this).find('option:selected').val();

                clearInterval(timer);
                if (_opt.time) {
                    timer = setInterval(function() {
                        _self.loadMore({
                            order: _opt.orderby + '|' + _opt.ordertype,
                            is_append: false

                        });
                    }, _opt.time);
                }
            })
            .off('click.loadMore').on('click.loadMore', _opt.loadMore, function() {
                page = $list.find('[data-cid]').eq(-1).data();

                _self.loadMore({
                    order: _opt.orderby + '|' + _opt.ordertype,
                    pageIndex: page.pageIndex + 1,

                })
            });
        }

    })

});