...
+function ($) {
    ...
    $.fn.ajaxBind = function (params, data) {
        var format = function format(data, field) {
            var result = [];
        ...
        var sub = function sub(data, field) {
            if (!data.sub) return [""];
            return format(data.sub, field);
        };

        var getSecondArr = function getSecondArr(d, field) {
        ...
        var defaults = {
            ...
            formatValue: function formatValue(picker, values, displayValues) {
                // 返回显示内容
                if (displayValues) {
                    var showVal = '';
...
define(['ajax'], function (require, exports, module) {
    ...
    $('.bar-tab .tab-item').removeClass('active').siblings('a[href="' + (decodeURI(location.pathname.slice(1)) || 'index.html') + '"]').addClass('active');
    $(document).off('.sm'
    ...
    //以下代码为ajax提交,使用form提交, 按钮增加id为btn即可
        ...
        ajax({
            ...
            dataType: 'json',
            success: function success(result, that) {
                if (result.status) {
                    $.toast(result.info);
                    if (result.url) {
    ...
    var modules = {
        ...
        // },
        // 手机收藏
        collect: function collect(_collect) {}
