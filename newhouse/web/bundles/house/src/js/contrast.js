 ...
 * 这是html上的对比按钮的结构
 * ```html
 * <input type="checkbox" data-contrast="1" data-contrast-cfg="{obj:this, pro_name:'{{item.name|default('')}}', pro_link:'{{('houses/detail')|U({id:item.id|default('')})}}', pro_id:'{{item.id|default('')}}'}" value="" name="" id="pro_{{item.id|default('')}}">
 * ```
 *
 * @说明
 ...
 * var chid = '4';
 * var comp_action= '{{("houses/contrast")|U({fid: 8})}}' + '&chid=' + chid + '#aids=';
 * ```
 *
 * @模板一 最外层的div
