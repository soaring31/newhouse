(function(window, document, $, undefined) {
	var DragResize = function(target, options) {
	...
	DragResize.prototype = {
	    ...
	    init : function(target, options) {
			...
			this.offsetParent = this.target.offsetParent || 'body';
			this.parentOffset = $(this.offsetParent).offset();
			this.parentOffset.left+= parseInt($(this.offsetParent).css('border-left-width'))||0;
			this.parentOffset.top += parseInt($(this.offsetParent).css('border-top-width')) ||0;
		...
		setOptions: function(options) {
			this.option = {
				...
				bound : {// 默认为限制在target.offsetParent内
					...
					right :this.parentOffset.left+ $(this.offsetParent).width(), 
					bottom:this.parentOffset.top + $(this.offsetParent).height()
				},   
		...
		onResizing: function(e){
			...
			$(this.target).trigger('resizing.dragResize');
			return false;
		},	
	...
	$.fn.dragResize = function(options) {
	    return this.each(function () {
	      var $this = $(this)
	      var data  = $this.data('dragResize')

	      if (!data) $this.data('dragResize', (data = new DragResize(this, options)))
