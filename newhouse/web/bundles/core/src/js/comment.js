...
define([
    ...
    (function(out) {
        ...
        init: function(opt) {
            ...
            $list.off('.cmt').on('click.cmt', _opt.replyBtn, function() {
                ...
                if (!$formClone.hasClass('inited')) {
                    ...
                    $formClone
                        .off('.yzm')
                        .on('click.yzm', '[data-code-codetext="1"]', function() {
                    ...
                    // 重置表单验证
                    $formClone.removeClass('form-origin')
                        .find('form')
                        .attr({
                            errorLabelContainer: function(i, v) {
        ...
        // 点评权限
        checkComment: function(saveUrl) {
           // var saveUrl = 'interaction/commentsave';
            $.ajax({
                url: BASE_URL + 'Critique/critiquemanage',
                type: 'GET',
                dataType: 'JSON'
            })
            .done(function(msg) {
                // console.log(msg)
                if (msg.allow == 1) {
                    $('.comment').addClass('comment-allow');
                    return false;
                } else {
                    $('.comment').addClass('comment-disallow');
                }
            })
            .fail(function() {
                console.log("error");
