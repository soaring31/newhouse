...
+function ($) {
    ...
    $.modal = function (params) {
        ...
        modalHTML = '<div data-tag="' + extraClass + '" class="modal ' + extraClass + ' ' + noButtons + '"><div class="modal-inner">' + (titleHTML + textHTML + afterTextHTML) + '</div><div class="modal-buttons ' + verticalButtons + '">' + buttonsHTML + '</div></div>';
    ...
    // Action Sheet
    $.actions = function (params, extraClass) {
        ...
        modalHTML = '<div class="actions-modal '+ extraClass +'">' + buttonsHTML + '</div>';
        _modalTemplateTempDiv.innerHTML = modalHTML;
    ...
    $.openModal = function (modal, cb) {
        ...
        if (!isLoginScreen && !isPickerModal && !isToast) {
            if ($('.modal-overlay').length === 0 && !isPopup) {
                $(defaults.modalContainer).append('<div class="modal-overlay modal-overlay-'+ modal.data('tag') +'"></div>');
            } else if (modal.data('tag')) {
                $('.modal-overlay').addClass('modal-overlay-'+ modal.data('tag'))
            }
    ...
    $.closeModal = function (modal) {
        ...
        var isModal = modal.hasClass('modal'),
            ...
            isModalEx = modal.hasClass('modal-ex'),
            overlay = isPopup ? $('.popup-overlay') : $('.modal-overlay');
        if (isPopup){
            // if (modal.length === $('.popup.modal-in').length) {
                // overlay.removeClass('modal-overlay-visible');
            // }
            // 修改是为了防止popup里面的modal弹窗关闭不全的情况
            overlay.removeClass('modal-overlay-visible');
        ...
        modal.removeClass('modal-in').addClass('modal-out').transitionEnd(function (e) {
            ...
            if (isPopup || isLoginScreen || isPickerModal || isModalEx) {
    ...
    function handleClicks(e) {
        ...
        if (clicked.hasClass('open-popup')) {
            ...
            var popupModal = $.popup(popup);
            // 保存激活的按钮
            e.target && $(popupModal).data('btn', e.target)
        }
...
//     // var defaults = {};
//     // var data; // 数据源
//     // var default_choose = ''; // 默认选中
//     // var t; // 定时器设定
//     // var current_cate; // 当前分类
//     // var new_cate;
//     // var current_sub; // 当前下级
//     // var new_sub;

//     // var format = function(data, field) {
//     //     var result = [];
...
//     // var sub = function(data, field) {
//     //     if(!data.sub) return [""];
//     //     return format(data.sub, field);
//     // };

    

//     // $.fn.ajaxPicker = function(params) {
//     //     data = params.data;
//     //     return this.each(function() {
//     //         if(!this) return;
//     //         defaults = {
//     //             rotateEffect: false,  //为了性能
//     //             onChange: function (picker, values, displayValues) {
//     //                 current_cate = picker.cols[0].value;
//     //                 if (default_choose !== current_cate) {
//     //                     clearTimeout(t);
//     //                     var currentCircleName = [];
//     //                     var currentCircleId = [];
//     //                     for (var i = 0, len = data.length; i < len; i++) {
//     //                         if (data[i].pid == values[0]) {
//     //                             currentCircleName.push(data[i].name)
//     //                             currentCircleId.push(data[i].id)
//     //                         };
//     //                     };
//     //                     if (!currentCircleName.length) {
//     //                         currentCircleName = ['']
//     //                     };
//     //                     if (!currentCircleId.length) {
//     //                         currentCircleId = [0]
//     //                     };
//     //                     t = setTimeout(function(){
//     //                         picker.cols[1].replaceValues(currentCircleId, currentCircleName);
//     //                         default_choose = current_cate;
//     //                         $('#' + params.ele).val(currentCircleId[0]);
//     //                         new_sub = currentCircleId[0];
//     //                         current_sub = new_sub;
//     //                         picker.updateValue();
//     //                     }, 200);
//     //                     return;
//     //                 };
//     //                 new_sub = picker.cols[1].value;
//     //                 if (current_sub !== new_sub) {
//     //                     $('#' + params.ele).val(values[1]);
//     //                     current_sub = new_sub;
//     //                     picker.updateValue();
//     //                 };
//     //                 var html = ',';
//     //                 for (var i = 0, len = displayValues.length; i < len; i++) {
//     //                     if (displayValues[i]) {
//     //                         html += ',' + displayValues[i]
//     //                     };
//     //                 };
//     //                 $(params.showEle).html(html.replace(',,', ''))
//     //             },
...
//     //                     return_val = values[0];
//     //                 } else {
//     //                     return_val = values[values.length - 1];
//     //                 };
//     //                 return return_val
//     //             }
//     //         }
//     //         var p = $.extend({}, defaults, params.picker);
//     //         $(this).picker(p);
//     //     });
//     // };

//     // $.fn.ajax1Picker = function(params) {
//     //     data = params.data;
//     //     console.log(data)
//     //     var cate_name = [];
//     //     var cate_id = [];
//     //     var sub_init_name = sub(data[0], 'name');
//     //     var sub_init_id = sub(data[0], 'id');

//     //     for (var i = 0, len = data.length; i < len; i++) {
//     //         cate_name.push(data[i].name);
//     //         cate_id.push(data[i].id);
//     //     };

//     //     current_cate = cate_id[0]
//     //     current_sub = sub_init_id[0];

//     //     var getSubName = function(d, field) {
//     //         for(var i = 0, len = data.length; i < len; i++) {
//     //             if(data[i].id == d) return sub(data[i], field);
//     //         }
//     //         return [""];
//     //     };
...
//     //                 if (default_choose !== current_cate) {
//     //                     clearTimeout(t);
//     //                     var currentCircleName = getSubName(current_cate, 'name');
//     //                     var currentCircleId = getSubName(current_cate, 'id');
//     //                     t = setTimeout(function(){
//     //                         picker.cols[1].replaceValues(currentCircleId, currentCircleName);
//     //                         default_choose = current_cate;
//     //                         // $('#' + params.ele).val(currentCircleId[0]);
...
//     //                 {
//     //                     textAlign: 'center',
//     //                     displayValues: cate_name,
//     //                     values: cate_id
//     //                 },
//     //                 {
//     //                     textAlign: 'center',
//     //                     displayValues: sub_init_name,
//     //                     values: sub_init_id
//     //                 }
//     //             ],
//     //             formatValue: function (picker, values, displayValues) {
...
//     //         if (p.value) {
//     //             if(p.value[0]) {
//     //                 current_cate = p.value[0];
//     //                 p.cols[1].values = getSubName(current_cate, 'id');
//     //                 p.cols[1].displayValues = getSubName(current_cate, 'name');
//     //             }
//     //         }
//     //         console.log(p);
//     //         $(this).picker(p);
...
+ function($) {
    ...
    $.fn.ajaxBind = function(params, data) {
        var format = function(data, field) {
            var result = [];
            for(var i = 0; i < data.length;i++) {
                var d = data[i];
                result.push(d[field || 'name']);
            }
            if(result.length) return result;
            return [""];
        };
        ...
        var getSecondArr = function(d, field) {
            for(var i = 0; i < raw.length; i++) {
                if(raw[i].id == d) return sub(raw[i], field);
            }
            return [""];
        };

        var raw = data;
        var firstCateNames = raw.map(function(d) {
            return d.name;
        });
        var firstCateIds = raw.map(function(d) {
            return d.id;
        });
        var initSecondName = sub(raw[0]);
        var initSecondIds = sub(raw[0], 'id');

        var currentProvince = firstCateNames[0]; // 一级当前数据
        var currentSecond = initSecondName[0]; // 二级当前数据
        var newSecondIds; // 新二级id合集
        var t; // 延时定义
        var firstId; // 一级提交id
        var secondId; // 二级提交id
        var showId; // 显示内容id
        var defaults = {
            ...
            onChange: function (picker, values, displayValues) {
                var newFirstCate = picker.cols[0].value;
                var newSecond;
                // 以及变化执行
                if(newFirstCate !== currentProvince) {

                    clearTimeout(t);

                    t = setTimeout(function(){
                        var newSecondNames = getSecondArr(newFirstCate);
                        newSecondIds = getSecondArr(newFirstCate, 'id');
                        newSecond = newSecondNames[0];
                        picker.cols[1].replaceValues(newSecondIds, newSecondNames);
                        currentProvince = newFirstCate;
                        currentSecond = newSecond;
                        $('#' + firstId).val(currentProvince);
                        if (secondId) {
                            $('#' + secondId).val(0)
                        };
                        picker.updateValue();
                ...
                // 二级变化执行
                newSecond = picker.cols[1].value;
                if(newSecond !== currentSecond) {
                    currentSecond = newSecond;
                    // 如果是同模型中的二级条件，则只更新一级提交id
                    if (secondId) {
                        $('#' + secondId).val(currentSecond)
                    } else {
                        $('#' + firstId).val(currentSecond);
                    };
                    picker.updateValue();
                }
            },
            ...
            cols: [
            {
                textAlign: 'center',
                values: firstCateIds,
                displayValues: firstCateNames,
            },
            {
                textAlign: 'center',
                values: initSecondIds,
                displayvalues: initSecondName,
            }
            ],
            formatValue: function (picker, values, displayValues) {
                // 返回显示内容
                if (displayValues) {
                    var showVal = '';
                    $.each(displayValues, function(index, el) {
                        if (el === '请选择') {
                            return false;
                        } else {
                            showVal += el + ',';
        ...
        return this.each(function() {
            ...
            if (p.value) {
                if(p.value[0]) {
                    ...
                    p.cols[1].displayValues = getSecondArr(p.value[0]);
                    p.cols[1].values = getSecondArr(p.value[0], 'id');
                }
                if(p.value[1]) {
                    currentSecond = p.value[1];
                }
            }
            ...
            showId = thisData.show;
            if (thisData.first === thisData.second) {
                firstId = thisData.first;
            } else {
                firstId = thisData.first;
                secondId = thisData.second;
            };
...
+function($){
    ...
    $.fn.swiper = function (params) {
        var _params = $.extend(true, {}, params, $(this).data());
        
        return new $.Swiper(this, _params);
    };
