...
define(['ajax', 'utils'], function (require, exports, module) {
    ...
    var app = {
        formInit: function formInit(opt) {
            var _self = this;
            var _opt = $.extend({}, {
                type: 'POST',
                datatype: 'json',
            ...
            // 提交表单
                ...
                if (!$submit.hasClass('requesting')) {
                    ...
                    console.log({
                        ...
                        type: eldata.type || _opt.type || $form.attr('method'),
            ...
            // 发送短信
                ...
                var params = utils.parseJSON(this_data.params) || {};
                if (role == 'tel') {
                    params.tel = codeVal;
                } else if (role == 'mail') {
                    params.mail = codeVal;
                ...
                if (!$this.hasClass('requesting')) {
                    ...
                    ajax({
                        type: 'post', // 短信发送此方式限定只能接受GET方式传送的参数,不要更改成POST
                        url: this_data.src,
                        data: params,
                        dataType: _opt.datatype,
                        jsonpCallback: _opt.jsonpCallback,
        ...
        validate: function validate($form) {
            ...
            $form.find(['select', 'textarea', '[type="text"]', '[type="password"]', '[type="file"]', '[type="number"]', '[type="search"]', '[type="tel"]', '[type="url"]', '[type="email"]', '[type="datetime"]', '[type="date"]', '[type="month"]', '[type="week"]', '[type="time"]', '[type="datetime-local"]', '[type="range"]', '[type="color"]', '[type="radio"]', '[type="checkbox"]', 'input[type="radio"]', 'input[type="checkbox"]'].join(',')).each(function (index, el) {
        ...
        toTop: function toTop() {
            ...
            $toTop.off('.top').on('click.top', function () {
                $content.scrollTo({ scrollTop: 0 });
            });
            $content.off('.top').on('scroll.top', function () {
        ...
        */
        adcount: function adcount(opt) {
            ...
            $(document).off('click.ad').on('click.ad', '[data-ad]', function (event) {
                var $this = $(this);
