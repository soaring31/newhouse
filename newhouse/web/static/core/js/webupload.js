    ...
    var out = {
        init: function init(o) {
            ...
            uploader.on('fileQueued', function (file) {
                ...
                // 图集
                if (_opt.pick.multiple) {
                    $li.prepend('<input name="' + _opt.name + '[]" value="" type="hidden">').append('<div class="textarea"><textarea placeholder="请输入描述" name="' + _opt.name + 'desc[]"></textarea></div>');
                };
            ...
            uploader.on('uploadSuccess', function (file, res) {
                if (res.msg && res.status) {
                    ...
                    $li.find('img').attr('src', _self.parseImgUrl(res.msg)).load(function () {
                        $li.find('.progress').remove();
                    });
                    $toolbar.removeClass('error').html('<i title="查看大图" class="preview font-ico-38"></i>\
        ...
        // 更新图片数量用于js表单验证
        updateImgNum: function updateImgNum($wrap) {
            try {
        ...
        parseImgUrl: function parseImgUrl(url) {
            if (!/http/.test(url)) {
        ...
        events: function events() {
            var _self = this;
            var _opt = _self.opt;
            _self.$wrap.off('.upload').on('mouseover.upload', '.file-item', function () {
            ...
            // 相册浏览
            );$(document).off('.imgs-gallery').on('mousemove.imgs-gallery', '.imgs-lay', function (e) {
                ...
                var run = function run(item) {
                    ...
                    if (newUrl) {
                        $this.data('url', newUrl).find('img').attr('src', _self.parseImgUrl(newUrl));
