...
define(['mapLibs', 'utils', 'template', 'jqmodal', 'filter', 'pagination'], function (require, exports, module) {
    ...
    var Mapview = function Mapview() {
        ...
        filter.init({
            callBack: function callBack(el) {
                ...
                if (moreLen) {
                    $('[data-more-num]').text('(' + moreLen + ')').closest('li').addClass('active');
                } else {
                    $('[data-more-num]').text('').closest('li').removeClass('active');
                }
                // 显示标注
    ...
    Mapview.prototype = {
        ...
        events: function events() {
            ...
            .on('click.map-complex', '[data-name="region"]', function (e) {
                ...
                if (!$this.closest('[data-filter-item-select]').length) {
                    var $sameThis = $('[data-filter-item-select]').find('[data-name="region"][data-value="' + $this.data('value') + '"]');
        ...
        list: function list(markerList, data, page) {
            var _self = this;
            var $map = map.$map;
            var zoom = map.zoom;
            ...
            var title;
            //  注意顺序aid放后面
            $('.act[data-name="region"]').not('.default').each(function () {
                title = $(this).data('title');
            });
            $('.act[data-name="aid"]').each(function () {
                title = $(this).data('title');
            });

            $('[data-list-top]').html(template(listOpt.tplListTop.slice(1), {
                title: title,
                num: page.pageCount
            }));
            // 列表
        ...
        page: function page(_page) {
            ...
            var pageselectCallback = function pageselectCallback(p) {
                //动态更新数据
                _self.drawItem({ pageIndex: p + 1 });
                return false;
            };

            $page.pagination(_page.pageCount, {
                num_edge_entries: 1, //边缘页数
                num_display_entries: 3, //主体页数
                callback: pageselectCallback,
                items_per_page: _page.pageSize, //每页显示1项
                current_page: _page.pageIndex - 1,
                prev_text: '前一页',
                next_text: '后一页',
...
//# sourceMappingURL=http://localhost:8888/public/js/map-complex.js.map
