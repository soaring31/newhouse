...
define('manage', [
    ...
    Manage.prototype = {
        ...
        main: function(opt) {
            ...
            _self.events();
            // 侧栏
            $('[data-aside="1"]').addClass(utils.localStorage.getItem('manage_aside') || null);
            // 清除所有后台缓存
            utils.localStorage.removeItem(/manage_/);
            // 路由中转链接
            _self.router();
            ...
            $(document)
                ...
                .on('keyup.manage', function(e) {
                    ...
                    $.each(aKeys, function(i, key) {
                        if (e[key]) aNewKeys.push(key.replace('Key', ''))
                    });
        ...
        middleware: function(e) {
            ...
            if (eldata.role == 'route' || eldata.role == 'jump') {
                eldata.target = eldata.target || $this.closest('[data-target]').data('target');
            }
            if (eldata.role == 'submit') {
                var $form = $this.closest('form');
                if ($form.length && !$this.attr('data-url')) {
                    eldata.url = $form.attr('action');
                }
            }

            var url = eldata.url || $this.attr('href') &&
                        !$this.attr('href').match(/^#+/) &&
                        $this.attr('href') || undefined;

            var _url = utils.parseURL(url);
            // debugger;
            eldata.data = utils.parseJSON(eldata.data || eldata.params);
            delete _url.params.area;
            // 合并配置
            eldata = $.extend(true, {}, this.param, eldata, {
                $el: $this,
                data: eldata.data,
                url: url && _url.source || undefined
            }, {
                data: _url.params
            });
            _url = null;
         ...
         * @param  {araay} aTags=null 要排除的条件字段
         * @return {Object}     json数据
         * @示例 调用
         * ```js
         * getData(['page']) // 获取排除分页的查询条件
         * ```
         */
        getData: function(oTags) {
            var oFilter = {
                   filter:  '.on[data-role="filter"]',
                   select:  'select[data-role="select"]',
                   search:  'input[data-role="search"]'
                },
                aFilter = [];

            $.each(oFilter, function(k, v) {
                if ($.inArray(k, oTags) < 0) {
                    aFilter.push(v);
                }
            });

            var defData = this.$area.find(aFilter.join(',')).serializeObject();

            if ($.inArray('page', oTags) < 0) {
                defData.pageIndex = this.$area.find('[data-page]').data('pageIndex');
            }

            return $.extend(true, {}, this.param.data, defData);
        },
        /**
         * @method sort
         * @description 排序选择器
        ...
        sort: function(eldata) {
            ...
            eldata.$el.data({
                ...
                .siblings('.act').data('value', '').removeClass('asc desc act');

            eldata.data = _self.getData(['page']);
         ...
         * @param  {Object} eldata data数据
         * @param  {String} eldata.role 选择器类型
         * @param  {String} eldata.name 要检索的字段
         * @param  {String} eldata.value 要检索的字段对应的值
         * @其它参数 见`select`方法
         * @示例 调用
         * ```html
         *   <a href="" data-role="filter" data-name="caid" data-value="2">国内资讯</a>
         * ```
         */
        filter: function(eldata) {
            if (eldata.$el.closest(this.param.cate).length) {
                this.menuClasschange(eldata.$el);
            } else {
                // 设置页面上的条件
                eldata.data = eldata.$el.serializeObject();
                eldata.target = this.param.cate;
            }
            // debugger;
            this.flush(eldata);
        },
        ...
        page: function(pageselectCallback) {
            ...
            pageselectCallback = pageselectCallback || function(page) {
                //动态更新数据
                $page.data('pageIndex', page + 1);
                eldata.data = _self.getData();
                _self.flush(eldata);
                return false;
            };
            ...
            // debugger;
            //中部菜单翻页效果
            eldata.$el.pagination(eldata.pageCount, {
                num_edge_entries    : 1, //边缘页数
                num_display_entries : 10, //主体页数
                callback            : pageselectCallback,
                items_per_page      : eldata.pageSize, //每页显示1项
                current_page        : ($page.data('pageIndex') || 1) - 1,
                prev_text           : '前一页',
                next_text           : '后一页',
         ...
         * @param  {jquery} $target 从哪个页面找`data-param`
         * @param  {Boolean} reset=0 是否重置`tableFixed` `page` `resizeWindows`
         * @示例 调用
         * ```js
         *   setParam($target)
         * ```
        ...
        getSetting: function(opt) {
            ...
            var url = opt.url || opt[opt.role + 'Url'] || opt['defUrl'];
            // debugger;
            var oSetting = $.extend(true, {}, Default, {
                ...
                type : opt.type,
                // 有模板时一定是json
                dataType: opt.tpl ? 'json' : opt.dataType
            });
            // debugger;
            return oSetting;
        },
         ...
         * @param {Object} opt 刷新配置
         * @param {Object} opt.data ajax的data
         * @param {String} opt.url ajax的url
         * @param {String} opt.type=GET ajax的type
         * @param {String} opt.dataType=html ajax的dataType
         * @param {funtion} done `ajax.done`，可覆盖默认函数
         * @param {funtion} fail `ajax.fail`，可覆盖默认函数
         * @param {funtion} always `ajax.always`，可覆盖默认函数
         */

        flush: function(opt, done, always) {
            var _self = this;

            var setting = _self.getSetting(opt);

            opt.target = opt.target || _self.param.list;

            // console.log(_self.$area);
            var $target = _self.$area.find(opt.target);
            // debugger;
            return $.ajax(setting)
                .done(done || function(d) {
                    try {
                        $target.empty().html($(d).find(opt.target).html());
                    } catch (err) {
                        console.warn(err);
                    }
                })
        ...
        show: function(eldata) {
            ...
            // 获取弹窗大小
            if (utils.localStorage.getItem('manage_popSize')) {
                aPopSize = utils.localStorage.getItem('manage_popSize').split(',');
            }
            // iframe
            if (eldata.iframe) {
                $.jqModal.modal({
            ...
            .on('resizing.dragResize', function() {
                ...
                utils.localStorage.setItem('manage_popSize', aSize);
            })
        ...
        ajaxAbort: function(ajaxKey) {
            ...
            if (ajaxKey && _self.xhrPool[ajaxKey]) {
                ...
                // 终止全部
                $.each(_self.xhrPool, function(key, jqXHR) {   //  cycle through list of recorded connection
        ...
        submit: function(eldata) {
            var _self = this,
                $form = eldata.$el.closest('form');
            var data;
            var ajaxUrl = eldata.url || _self.param.submitUrl;
                    ...
                    if (d.status) {
                        if (d.jumpurl) {
                            ...
                            if (eldata.reload) {
                                if (eldata.reload != 1) {
                                    // 刷新指定区域
                                    _self.reload(eldata.reload);
                                } else {
                                    // 刷新默认区域
                                    // 仅刷新
                                    var isAdd = !$form.data('id');
                                    // add时不需要保留分页
                                    _self.reload(eldata.target, isAdd ? ['page'] : []);
                                }
                            }
                        } else {
                            // 列表时 仅刷新
                            _self.reload(eldata.target);
                        }
                    }
                })
                .always(function() {
            ...
            if ($form.length) {
                ...
                var serializeDataParams = utils.objectToSerializeArray(eldata.params);
                // 注意反转，保存数据如果有相同的name时后面的起作用
                var serializeData = $form.find('input, select, textarea').serializeArray().reverse();
                // 过滤掉name相同的数据
        ...
        route: function(eldata) {
            ...
            if (eldata.routeType == 'onlyHashChange') {
                ...
                _self.menu(eldata.$el, function() {
                    // 触发下级
                    _self.triggerNext($(eldata.target));
            ...
            var parseRoute = function() {
                ...
                try {
                    if (!arrRouteParams.length) {
                        arrRouteParams = ['mindex'];
                    } else if (arrRouteParams[0] == 'default') {
                        // 第一级是左侧时
                        arrRouteParams.shift();
                    }
                    // 保存下当前的路由，手动点击二级菜单时要用
                    _self.arrRouteParams = arrRouteParams;
        ...
        delete: function(eldata) {
            ...
            if ($row.length) {
                // 单条
                eldata.data.id = $row.data('id');
            } else {
                // 批量
                eldata.data.id = _self.getListData();
            }
            ...
            if (!eldata.data.id) {
                ...
                $.jqModal.confirm({
                    ...
                    accept: function() {
                        $.ajax({
                                url: eldata.url || eldata.deleteUrl,
                                data: eldata.data,
                                type: 'DELETE'
                            })
                            .done(function(d) {
        ...
        // field 需要排除的条件
        reload: function(target, field) {
            var _self = this;
            // var aOnIndex = _self.$area.find(_self.param.list + ' .on').map(function() {
            //     return $(this).index();
            // }).get();
            // 刷新都找不到时找param刷新
            // console.log(target, _self.param.list, '[data-param]');
            var newTarget = target || _self.param.list || '[data-param]';
            var $newTarget = _self.$area.find(newTarget);
            // var isMenu = $newTarget.find('[data-level="top"]').length;
            // 临时处理
            var isMenu = 0;
            _self.flush({
                url: _self.param.defUrl,
                target: newTarget,
                data: _self.getData(field),
                type: 'GET',
                dataType: 'html'
            }, isMenu ? function(d) {
                // 如果是菜单刷新单条数据
                    // 当前行
                    var $tRow = $newTarget.find('.is-active-btn').closest('tr');
                    // 当前行的选择标识
            ...
            // .done(function() {
            //     var $trs = _self.$area.find(_self.param.list + ' .on').eq(0).siblings('.on').andSelf();
            //         console.log($trs);
            //     $.each(aOnIndex, function(i, v) {
            //         $trs.eq(v).addClass('on');
            //     });
            // });
        },
        /**
