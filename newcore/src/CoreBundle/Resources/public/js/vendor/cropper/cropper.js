...
(function($) {
	...
	function getSourceCanvas(image, data) {
		...
		var scalable = isNumber(scaleX) && isNumber(scaleY)
				&& (scaleX !== 1 || scaleY !== 1);
	...
	function Cropper(element, options) {
		...
		this.options = $.extend({}, Cropper.DEFAULTS, $.isPlainObject(options)
				&& options);
		this.isLoaded = false;
		this.isBuilt = false;
	...
	Cropper.prototype = {
		...
		load : function(url) {
			...
			if (REGEXP_DATA_URL.test(url)) {
				return REGEXP_DATA_URL_JPEG.test(url) ? read(dataURLToArrayBuffer(url))
						: this.clone();
			}
		...
		clone : function() {
			...
			this.$clone = $clone = $('<img' + getCrossOrigin(crossOrigin)
					+ ' src="' + (crossOriginUrl || url) + '">');
			...
			if (this.isImg) {
				...
				$clone.one(EVENT_LOAD, $.proxy(this.start, this)).one(
						EVENT_ERROR, $.proxy(this.stop, this)).addClass(
						CLASS_HIDE).insertAfter($this);
			}
		},
		...
		start : function() {
			...
			getImageSize($image[0], $.proxy(function(naturalWidth,
					naturalHeight) {
				$.extend(this.image, {
		...
		renderCanvas : function(isChanged) {
			...
			canvas.oldLeft = canvas.left = min(
					max(canvas.left, canvas.minLeft), canvas.maxLeft);
			canvas.oldTop = canvas.top = min(max(canvas.top, canvas.minTop),
					canvas.maxTop);
		...
		unbind : function() {
			...
			$document.off(EVENT_MOUSE_MOVE, this._cropMove).off(EVENT_MOUSE_UP,
					this._cropEnd);
		...
		cropEnd : function(event) {
			...
			if (action) {
				...
				if (this.cropping) {
					this.cropping = false;
					this.$dragBox.toggleClass(CLASS_MODAL, this.isCropped
							&& this.options.modal);
				}
		...
		replace : function(url, onlyColorChanged) {
			if (!this.isDisabled && url) {
				...
				if (onlyColorChanged) {
					...
					if (this.isBuilt) {
						this.$preview.find('img').add(this.$clone2).attr('src',
								url);
					}
				} else {
					if (this.isImg) {
		...
		move : function(offsetX, offsetY) {
			...
			this.moveTo(isUndefined(offsetX) ? offsetX : canvas.left
					+ num(offsetX), isUndefined(offsetY) ? offsetY : canvas.top
					+ num(offsetY));
		},
		...
		zoomTo : function(ratio, _event) {
			...
			if (ratio >= 0 && this.isBuilt && !this.isDisabled
					&& options.zoomable) {
		...
		rotateTo : function(degree) {
			...
			if (isNumber(degree) && this.isBuilt && !this.isDisabled
		...
		setCropBoxData : function(data) {
			...
			if (this.isBuilt && this.isCropped && !this.isDisabled
					&& $.isPlainObject(data)) {
		...
		getCroppedCanvas : function(options) {
			...
			context.drawImage
					.apply(context,
							(function() {
								var source = getSourceCanvas(this.$clone[0],
										this.image);
		...
		setDragMode : function(mode) {
			...
			if (this.isLoaded && !this.isDisabled) {
				...
				this.$dragBox.data(DATA_ACTION, mode).toggleClass(CLASS_CROP,
						croppable).toggleClass(CLASS_MOVE, movable);

				if (!options.cropBoxMovable) {

					// Sync drag mode to crop box when it is not movable(#300)
					this.$face.data(DATA_ACTION, mode).toggleClass(CLASS_CROP,
							croppable).toggleClass(CLASS_MOVE, movable);
				}
			}
		}
	...
	Cropper.TEMPLATE = ('<div class="cropper-container">'
