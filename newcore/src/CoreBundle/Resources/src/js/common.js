...
define('common', [
    ...
    var app = {
        ...
        gotop: function() {
            ...
            $(document).off('.gotop')
                .on('click.gotop', '[data-gotop="1"]', function() {
         ...
         *        </div>
         *        <div class="login-out">
         *            <a href="{{ ('house/login')|U }}"  data-modal=".winlogin"  title="登录"><i class="font-icons-2"></i>
         *                登录
         *            </a>
         *            <a href="{{ ('register/index')|U }}" title="注册" target="_blank"><i class="font-icons-30"></i>
         *                注册
         *            </a>
         *        </div>
         *</script>
    ...
    var app1 = {
        form(opt) {
            ...
            $(document)
                .off('.formModal')
                .on('click.formModal', '.btn-jqModal-ex', function(e) {
                    var $modal = $form.filter($(this).attr('href'));
                    var config = _self.config = utils.parseJSON($modal.data('form-config'));

                    // 弹窗提示
                    $modal.html('<div style="text-align: center;padding: 20px"><i class="ico font-modal-load"></i>玩命加载中...</div>')
                        .jqModal(config.modal || {})

                    _self.formAjax($modal)
                        .always(function() {
                            // console.log("complete");
                            // 重新定位
