...
class Arrays extends ServiceBase
{
    /**
     ...
     * @since 1.0
     */
    public function _in_array( $needle, array $haystack, $strict = 0 )
    {
        ...
        if ( is_array($needle) && $strict === 2 )
        {
            ...
            {
                $return = self::_in_array($need, $haystack, $strict);
                if($return)
                    return $return;
            }
        }
        ...
        foreach($haystack as $value)
        {
            if( is_array($value) )
            {
                $return = self::_in_array($needle, $value, $strict);
                if($return)
                    return $return;
            }
     ...
     * @since 1.0
     */
    public function _array_push( array &$array, $key = '', $value, $Layer = 0 )
    {
        static $num = 1;
        $LayerAbs = abs($Layer);
     ...
     */
    public function setObjectDOM(&$object, $key, $value)
    {
        foreach($object as &$v)
        {
            $v->$key = $value;
     ...
     */
    public function _array_multisort(array &$array,$orderkey = 'vieworder',$keepkey = false)
    {
        $vorder = $eorder = array();
        if(!is_array($array) || empty($array) || !function_exists('array_multisort'))
            return;
     ...
     */
    public function _array_uasort( array &$array, $function = '__numberOfCharactersCmp' )
    {
     ...
     * 按字符数比较
     */
    private function __numberOfCharactersCmp( &$a, &$b )
    {
        if ( is_array($a) )
        {
     ...
     * @since  nv50
     */
    public function _array_multi_to_one ( array $arrays, $retentionKey = false )
    {
     ...
     * @since nv50
     */
    public function array_stripslashes(&$array)
    {
        if(is_array($array))
        {
     ...
     * @since  nv50
     */
    public function array_dimension(array $array)
    {
        $dimension = 0;
        if ( empty($array) )
        {
     ...
     * @since  nv50
     */
    public function limit( array $array, $limit, $offset = 0, $reservedKEY = false)
    {
        if ( empty($offset) )
        {
            $offset = $limit;
     ...
     * @param  array  		$array 				源数组
     */
    public function Set(&$array,$Key = '',$Value = 0){
