 ...
 * bd09ll（百度经纬度坐标）、bd09mc（百度摩卡托坐标）、gcj02（国测局加密坐标）、wgs84（gps设备获取的坐标）
 */
...
class Map extends BaseType implements MapInterface
{
    ...
    public function locationByIP($ip)
    {
        ...
        if (! filter_var($ip, FILTER_VALIDATE_IP))
            throw new \InvalidArgumentException('ip地址不合法');

        $params = array(
            'ak' => 'xrRoFHqALtMPNGXGIk6ONfKE',
            'ip' => $ip,
            'coor' => 'bd09ll'
        ) // 百度地图GPS坐标
;
