...
class ImageWatermark
{
	...
	{
		if ($this->type == 1) {
			$water = new Image($this->file);
			if (!$water->isImage() || !$water->getSource()) {
				return false;
			}
		} else {
			if (!$this->text || strlen($this->fontcolor) <=3)
				return false;

			empty($this->fontfamily) && $this->fontfamily = 'simsun.ttc';
			empty($this->fontsize) && $this->fontsize = 12;
			$this->fontfile =  'captcha' .DIRECTORY_SEPARATOR. $this->fontfamily;
	...
	public function doText($source, $water, $offsetX, $offsetY)
	{
		...
		imagettftext($source, $this->fontsize, 0, $offsetX, $offsetY + $water->height, imagecolorallocatealpha ($source, $R, $G, $B, 0), $this->fontfile, $this->text);
		unset($water);
		return $source;
	}
