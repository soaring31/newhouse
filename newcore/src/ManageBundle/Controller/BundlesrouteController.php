...
{    
    ...
    public function saveAction()
    {
        ...
        if(empty($key))
            return $this->error('唯一标识名称不能为空');

        if(isset($routeInfo[$key])&&empty($defaults))
            return $this->error('唯一标识名称已存在');
    ...
    {
        $bundleName = $this->get('request')->get('bundle', '');
        $key = $this->get('request')->get('routename', '');
    
        if(empty($bundleName)||empty($key))
            return $this->error('提交参数错误');
