...
class AopClient extends AopEncrypt
{
    ...
    public function pageExecute($request,$httpmethod = "POST")
    {
        ...
        if (method_exists($request,"getNeedEncrypt") &&$request->getNeedEncrypt()){
            ...
            // 执行加密
            $enCryptContent = self::encrypt($apiParams['biz_content'], $this->encryptKey);
    ...
    public function execute11($request, $authToken = null, $appInfoAuthtoken = null)
    {
        $this->setupCharsets($request);
        ...
        $requestUrl = self::normalizeUrl($requestUrl,$sysParams);

dump($sysParams, $apiParams);
        //发起HTTP请求
        try {
            $resp = $this->curl($requestUrl, $apiParams);
        } catch (\Exception $e) {
    ...
    private function encryptJSONSignSource($request, $responseContent) {
        ...
        $bizContent = self::decrypt($parsetItem->encryptContent, $this->encryptKey);
