...
class GenericOAuth2ResourceOwner extends AbstractResourceOwner
{
    ...
    {
        return $request->query->has('code')||$request->query->has('auth_code');
    }
    ...
    public function isCsrfTokenValid($csrfToken)
    {
        ...
        try {
            ...
            throw new \RuntimeException('Given CSRF token is not valid.');
        }
    }
     ...
     * {@inheritDoc}
     */
    protected function doGetTokenRequest($url, $parameters = array())
    {
        return $this->httpRequest($url, is_array($parameters)?http_build_query($parameters, '', '&'):$parameters);
    }
